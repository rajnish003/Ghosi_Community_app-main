...
return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-6 bg-green-200 min-h-[80vh]">
            {data.map((card) => (
                <div key={card.id} className="bg-white shadow-xl rounded-xl p-6 border border-gray-300 hover:shadow-2xl transition-transform transform hover:scale-105 flex flex-col items-center text-center">
                    <h3 className="text-2xl font-bold mb-2 text-gray-800">{card.title}</h3>
                    <p className="text-gray-600 mb-4">{card.description}</p>
                    <button className="px-4 py-2 bg-green-400 text-white rounded-lg shadow-md hover:bg-green-600 transition"
